...
import io.siuolplex.soul_ice.block.RujoneBerryBushBlock;
...
public class SoulIceBlocks {
    ...
    public static final Block ORANGE_ROSE = register("orange_rose", new FlowerBlock(StatusEffects.FIRE_RESISTANCE, 120, AbstractBlock.Settings.of(Material.PLANT, MapColor.ORANGE).breakInstantly().dynamicBounds().sounds(BlockSoundGroup.GRASS).noCollision()));

    public static AbstractBlock.Settings sharedSettings() {
        return Block.Settings.of(Material.DENSE_ICE, MapColor.LIGHT_BLUE).strength(3.0F).slipperiness(SoulIceConfig.slipperiness).sounds(BlockSoundGroup.GLASS);
    }
