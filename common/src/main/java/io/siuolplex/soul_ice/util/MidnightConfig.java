...
public abstract class MidnightConfig {
    ...
    public static void init(String modid, Class<?> config) {
        ...
        oldPath = Loader.getConfigDir().resolve("soul_ice_config" + ".json");
        ...
        for (Field field : config.getFields()) {
            ...
            if ((field.isAnnotationPresent(Entry.class) || field.isAnnotationPresent(Comment.class)) && !field.isAnnotationPresent(Server.class))
                if (Loader.getIsClient()) initClient(modid, field, info);
        ...
        try {
            ...
            try {
                gson.fromJson(Files.newBufferedReader(oldPath), config);
