...
public class GlutenBallItem extends SnowballItem {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        ...
        if (!level.isClientSide) {
            GlutenBallEntity snowball = new GlutenBallEntity(player, level);
            snowball.setItem(itemStack);
            snowball.shootFromRotation(player, player.getXRot(), player.getYRot(), level.getRandom().nextFloat() * 0.1F * (level.getRandom().nextBoolean() ? 1 : -1), 1.5F, 0.75F);
